...
@section('content')
    ...
    <script>
        ...
        function crearBotonVer(infoPiezas, i, usuarios) {
            ...
            a.className = "btn-pza";
            let url =
            "{{ route('piezaElegida', ['piezas' => ':piezas', 'proceso' => ':proceso', 'perfil' => ':perfil']) }}";
                        ...
                        @if (isset($array) && $array[0] != 'Todos')
                            ...
                            @foreach ($operadores as $operadores)
                                @if ($operadores->nombre . ' ' . $operadores->a_paterno . ' ' . $operadores->a_materno != $array[0])
                                    <option
                                        value="{{ $operadores->nombre }} {{ $operadores->a_paterno }} {{ $operadores->a_materno }}">
                                        {{ $operadores->nombre }} {{ $operadores->a_paterno }} {{ $operadores->a_materno }}
                                    </option>
                                @endif
                            @endforeach
                        @else
