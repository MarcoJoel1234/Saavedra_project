...
<body background="{{ asset('images/hola.jpg') }}">
    <div class="container">
            ...
            @csrf
            <input type="hidden" name="proceso" value="soldaduraPTA">
        ...
        @if (isset($band) && $band == 2)
            <div class="disabled-tabla">
                <form action="{{ route('soldaduraPTAHeader')}}" method="post">
                    @csrf
                ...
                @if (isset($nPiezas) && $nPiezas != "[]")
                    <form action="{{ route('editSoldaduraPTA')}}" method="post">
                        @csrf
