...
addSelect(window.workOrders, "workOrder", "Selecciona una orden de trabajo"); // Agregar select

function addSelect(array, name, labelText) {
    let container = name != "workOrder" ? document.querySelector(".row-selects") : document.querySelector(".row-workOrder");

    let div = document.createElement('div');
    div.className = `select-container div-${name}`;

    let objectKeys = array;
    if (name != "operation" && name != "subOperation") {
    ...
    let select = createSelect(options, name);
    ...
    select.addEventListener("change", function () {
        ...
        switch (select.name) {
            case "workOrder":
                addSelect(window.workOrders[select.value], "class", "Selecciona una clase");
                break;
            case "class":
                selectWO = document.querySelector('.select-workOrder');
                addSelect(window.workOrders[selectWO.value][select.value], "operation", "Selecciona un proceso");
                break;
            case "operation":
                //Crear el select de sub-proceso
                if (select.value == "1 y 2 Operacion Equipo") {
                    options = ["1 operacion", "2 operacion"];
                    addSelect(options, "subOperation", "Selecciona la operación");
                } else if (select.value == "Copiado") {
                    options = ["Cilindrado", "Cavidades"];
                    addSelect(options, "subOperation", "Selecciona el subproceso");
                } else {
...
function createSelect(options, name) {
    ...
    select.name = name;
    select.className = `select-${name}`; // Agregar clase al select

    //Agregar opcion deshabilitada
...
function removeSelect(name) {
    let nameSelects = ["class", "operation", "subOperation"];
    switch (name) {
        case "workOrder":
            removeButtonSearch();
            break;
        case "class":
            nameSelects.splice(0, 1);
            removeButtonSearch();
            break;
        case "operation":
            nameSelects.splice(0, 2);
            removeButtonSearch();
            break;
        case "subOperation":
            nameSelects.splice(0, 3);
            break;
    }
    if (nameSelects.length > 0) {
        nameSelects.forEach((nameSelect) => {
            let select = document.querySelector(`.div-${nameSelect}`);
            if (select != null) {
                select.remove();
            }
        });
...
function createButtonSearch() {
    ...
    if(button == null){
        ...
        button.className = "btn-search btn"; // Agregar clase al boton
        button.innerHTML = "Buscar";
        button.type = "submit";
        form.appendChild(button);
    }
}
function removeButtonSearch() {
