...
function createRowsForm(formInputs) {
    ...
    const numberRows = Number.isInteger(Object.keys(formInputs).length / 2) ? Object.keys(formInputs).length / 2 : Math.ceil(Object.keys(formInputs).length / 2);

    let inputsCounter = 0; //Contador para los inputs que se van insertando en el formulario
    for (let i = 0; i < numberRows; i++) { //For para la creacion del div "row" para cada par de inputs
        let nameInput = Object.keys(formInputs)[inputsCounter]; //Obtención del nombre del input
        if (nameInput != "table") {
            let row = document.createElement('div');
            row.className = "row";
            for (let j = 0; j < 2; j++) { //For para la creacion de los div "col" para cada input
...
function createSelectOrInput(element, attributesArray, nameInput) {
    ...
    if (element == "select") {
        ...
        if (nameInput == "classType") { //Si el select es el de tipo de clase, se añade un evento para modificar el select
...
function modifySelect(className) {
    ...
    if ((className == 'Obturador' && label != 'Seleccione la sección') || (className != 'Obturador' && label != 'Seleccione el tamaño')) {
...
function createSelect(className) {
    ...
    if (className == 'Obturador') { //Si la clase es obturador, se crea un select con las opciones de sección
    ...
    } else { //Si la clase no es obturador, se crea un select con las opciones de tamaño
...
function createScrollableTable(classes = null) {
    ...
    if (classes != null) { //Si no se reciben las clases, se muestra un mensaje de alerta
...
function createTableClasses(classes) {
    ...
    let titles = ["Clase", "Tamaño/Sección", "Piezas con consignación", "Pedido"];
    ...
    //Creación de la fila de títulos
    classes.forEach((classArray) => { //Se recorren las clases
        let button = document.createElement('button');
        button.value = classArray['id'];
        button.className = "btnClass";

        for (let field in classArray) { //Se recorren los campos de cada clase
            switch (field) { //Switch para insertar los campos correspondientes en la tabla
                case 'nombre':
                case 'tamanio':
                case 'seccion':
                case 'piezas':
    ...
    if (!action) {
        if (idClass !== null) {
            ...
            createButtons(idClass).forEach((button) => {
                if (window.profile == 5 && button.innerHTML == "Eliminar") {
...
function createButtons(idClass) {
    ...
    btn_deleteClass.innerHTML = "Eliminar";
    ...
    btn_editClass.innerHTML = "Editar";
...
function enableEditClass(idClass) {
    ...
    for (let classObject in window.classes) {
        if (window.classes[classObject].id == idClass) {
            div_rowsHidden.appendChild(createRowsForm(get_inputAttributes(window.workOrder.id, window.molding.nombre, window.classes[classObject])[1]));
            break;
        }
    }
        ...
        if (classesObject[classObject].id == classSelected) {
            let formInputs = {
                ...
                size: {
                    ...
                    input: {
                        type: "text",
                        value: classesObject[classObject].tamanio == null ? classesObject[classObject].seccion : classesObject[classObject].tamanio,
                        disabled: true
                    },
                },
                order: {
                ...
                finishDate: {
                    ...
                    input: {
                        type: 'date',
                        value: classesObject[classObject].fecha_termino == null ? "" : classesObject[classObject].fecha_termino,
                        disabled: true
                    }
                },
                finishTime: {
                    ...
                    input: {
                        type: 'time',
                        value: classesObject[classObject].hora_termino == null ? "" : classesObject[classObject].hora_termino,
                        disabled: true
                    }
                }
            };
...
function createCheckboxAddClass() {
    ...
    checkbox.addEventListener('change', function () {
        ...
        if (checkbox.checked) {
            ...
            let div_rowsHidden = document.querySelector('.div-rows-hidden');
            div_rowsHidden.innerHTML = "";
            div_rowsHidden.appendChild(createRowsForm(get_inputAttributes(window.workOrder.id, window.molding.nombre)[1]));
...
function get_operationsArray(className) {
    ...
    switch (className) {
        case "Bombillo":
        case "Molde":
            operations = ['Cepillado', 'Desbaste exterior', 'Revision Laterales', '1ra Operación',
                'Barreno maniobra', '2da Operación', 'Soldadura', 'Soldadura PTA', 'Rectificado',
                'Asentado', 'Calificado', 'Acabado ' + className, 'Barreno profundidad', 'Cavidades', 'Copiado',
                'Offset', 'Palomas', 'Rebajes', 'Grabado'
            ];
            operationsArray = ["cepillado", "desbaste_exterior", "revision_laterales", "pOperacion",
                "barreno_maniobra", "sOperacion", "soldadura", "soldaduraPTA", "rectificado", "asentado",
                "calificado", "acabado" + className, "barreno_profundidad", "cavidades", "copiado", "offSet",
                "palomas",
                "rebajes", "grabado"
            ];
            break;
...
function crearCasillas(operations, operationsArray, markedProcesses, edit) {
    ...
    for (let i = 0; i < operations.length; i++) { //For para la creación de cada una de las casillas
...
function automateCheckbox(checkbox, machineInput, operationName, markedProcesses, edit) {
    ...
    if (markedProcesses !== null) { //Si el proceso ya ha sido seleccionado anteriormente en la clase, se muestra marcado el checkbox y se muestran las maquinas en el input
        checkbox.checked = false;
        machineInput.disabled = true;
    ...
    if (!edit) { //Si no se esta editando la clase, se deshabilita todo (Unicamente se muestran)
    ...
    if (window.profile == 5) {
        machineInput.disabled = true;
        checkbox.disabled = true;
        if (markedProcesses == null) { // Si el proceso no ha sido seleccionado anteriormente en la clase
            checkbox.checked = false;
        }
    }
...
function changeStatusCheckbox(checkbox, machineInput) {
    if (checkbox.checked) { //Si el checkbox se marca
        machineInput.disabled = false;
...
function mostrarDiv(route) {
    ...
    label.innerHTML = route.includes("Class") ? "¿Estás seguro de eliminar la clase?" : "¿Estás seguro de eliminar la orden de trabajo?";
