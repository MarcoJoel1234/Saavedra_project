...
class AcabadoBombilloController extends Controller
{
        ...
        if (count($ot) != 0) {
            ...
            foreach ($ot as $ot) {
                ...
                foreach ($clases as $clase) {
                    ...
                    if ($proceso) {
                        if ($proceso->acabadoBombillo) { //Si existen maquinas en cepillado de esa clase, se almacena en el arreglo que se pasara a la vista
                            $contador++;
                        }
                    }
                }
    ...
    public function storeheaderTable(Request $request)
    {
        ...
        $id = "acabadoBombillo_" . $clase->nombre . "_" . $ot->id; //Creación de id para tabla Cepillado.
        ...
        if (isset($request->n_pieza)) {  //Si se obtienen los datos de las piezas, se guardan en la tabla Cepillado_cnominal.
            ...
            if ($piezaExistente) {
                ...
                $piezaExistente->gauge_corona = $request->gauge_corona;
                $piezaExistente->gauge_llanta = $request->gauge_llanta;
                $piezaExistente->altura_total = $request->altura_total;
                $piezaExistente->diametro_boca = $request->diametro_boca;
                $piezaExistente->diametro_asiento_corona = $request->diametro_asiento_corona;
                $piezaExistente->diametro_llanta = $request->diametro_llanta;
                $piezaExistente->diametro_caja_corona = $request->diametro_caja_corona;
                $piezaExistente->profundidad_corona = $request->profundidad_corona;
                $piezaExistente->angulo_30 = $request->angulo_30;
                $piezaExistente->profundidad_caja_corona = $request->profundidad_caja_corona;
        ...
        if ($id_proceso !== "[]") {
            ...
            //Actualizar resultado de la meta
            $pzasMeta = AcabadoBombilo_pza::where('id_meta', $meta->id)->where('error', 'Ninguno')->get(); //Obtención de todas las piezas correctas.
            Metas::where('id', $meta->id)->update([ //Actualización de datos en tabla Metas.
        ...
        if (isset($request->n_pieza)) { //Si se obtienen los datos de las piezas, se gua
            for ($i = 0; $i < count($request->n_pieza); $i++) {
                ...
                if ($piezaExistente) {
                    ...
                    $piezaExistente->gauge_corona = $request->gauge_corona[$i];
                    $piezaExistente->gauge_llanta = $request->gauge_llanta[$i];
                    $piezaExistente->altura_total = $request->altura_total[$i];
                    $piezaExistente->diametro_boca = $request->diametro_boca[$i];
                    $piezaExistente->diametro_asiento_corona = $request->diametro_asiento_corona[$i];
                    $piezaExistente->diametro_llanta = $request->diametro_llanta[$i];
                    $piezaExistente->diametro_caja_corona = $request->diametro_caja_corona[$i];
                    $piezaExistente->profundidad_corona = $request->profundidad_corona[$i];
                    $piezaExistente->angulo_30 = $request->angulo_30[$i];
