...
class RectificadoController extends Controller
{
    public function show()
    {
        ...
        if (count($ot) != 0) {
            ...
            if (count($oTrabajo) != 0) {
                return view('processes.rectificado', ['ot' => $oTrabajo]); //Retorno a vista de  Rectificado
            }
            //Se retorna a la vista de Cepillado con las ordenes de trabajo que tienen clases que pasaran por Rectificado
            return view('processes.rectificado', ['ot']); //Retorno a vista de Rectificado
        }
        ...
        if (isset($request->n_pieza)) {  //Si se obtienen los datos de las piezas, se guardan en la tabla Rectificado_cnominal.
            ...
            if ($piezaExistente) {
                ...
                if (isset($pzaUtilizar)) { //Si existe una pieza para utilizar, se retorna a la vista de Rectificado
                    ...
                    return view('processes.rectificado', ['band' => 2, 'moldura' => $moldura->nombre, 'ot' => $ot, 'meta' => $meta, 'nPiezas' => $pzasCreadas, 'clase' => $clase, 'piezaElegida' => $pzaUtilizar, 'juegos' => count($this->piezaUtilizar($ot->id, $clase))]); //Retorno a vista de Cepillado.
                } else {
            ...
            if (!$proceso) {
                ...
                $soldadura = new Rectificado(); //Creación de objeto para llenar tabla Rectificado
        ...
        if ($id_proceso !== "[]") {
            ...
            if ($pzaUtilizar == null) { //Si no existe una pieza para utilizar, se retorna a la vista de Rectificado
                ...
                if (isset($pzasUtilizar)) { //Si no se encontro una pieza para utilizar, se crea una nueva pieza.
                    return view('processes.rectificado', ['band' => 2, 'moldura' => $moldura->nombre, 'ot' => $ot, 'meta' => $meta, 'nPiezas' => $pzasCreadas, 'clase' => $clase, 'piezasUtilizar' => $pzasUtilizar, 'juegos' => count($pzasUtilizar)]); //Retorno a vista de Rectificado
                } else {
                    return view('processes.rectificado', ['band' => 2, 'moldura' => $moldura->nombre, 'ot' => $ot, 'meta' => $meta, 'nPiezas' => $pzasCreadas, 'clase' => $clase, 'piezaElegida' => $pzaUtilizar, 'juegos' => count($this->piezaUtilizar($ot->id, $clase))])->with('success', 'Se han registrado todas las piezas correctamente'); //Retorno a vista de Cepillado.
                }
            } else {
                return view('processes.rectificado', ['band' => 2, 'moldura' => $moldura->nombre, 'ot' => $ot, 'meta' => $meta, 'nPiezas' => $pzasCreadas, 'clase' => $clase, 'piezaElegida' => $pzaUtilizar])->with('success', 'Se han registrado todas las piezas correctamente'); //Retorno a vista de Rectificado
            }
        }
    }
    public function edit(Request $request)
    {
        ...
        if (isset($request->n_pieza)) { //Si se obtienen los datos de las piezas, se guardan en la tabla Rectificado_cnominal.
            ...
            if (isset($pzasUtilizar)) { //Si existe una pieza para utilizar, se retorna a la vista de Rectificado
                ...
                $pzasUtilizar = $this->piezaUtilizar($ot->id, $clase); //Llamado a función para obtener las piezas disponibles.
                return view('processes.rectificado', ['band' => 2, 'moldura' => $moldura->nombre, 'ot' => $ot, 'meta' => $meta, 'nPiezas' => $pzasCreadas, 'clase' => $clase, 'piezasUtilizar' => array(), 'piezaElegida' => $pzaUtilizar, 'juegos' => count($pzasUtilizar)])->with('success', 'Se han registrado todas las piezas correctamente'); //Retorno a vista de Rectificado
            }
        } else {
            if (isset($request->password)) { //Si se ingreso una contraseña y la meta existe entonces...
                ...
                foreach ($usersPasswords as $userPassword) { //Recorro las contraseñas.
                    if (Hash::check($request->password, $userPassword->contrasena) && $userPassword->perfil == 1) {  //Si la contraseña es correcta.
                        return view('processes.rectificado', ['band' => 4, 'moldura' => $moldura->nombre, 'ot' => $ot, 'meta' => $meta, 'clase' => $clase, 'nPiezas' => $pzasCreadas, 'juegos' => count($this->piezaUtilizar($ot->id, $clase))]); //Retorno la vista de cepillado.
                    }
                }
            }
            $pzaUtilizar = Rectificado_pza::where('id_proceso', $id_proceso->id)->where('estado', 1)->where('id_meta', $meta->id)->first(); //Obtención de la pieza a utilizar.
