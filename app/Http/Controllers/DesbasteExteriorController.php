...
class DesbasteExteriorController extends Controller
{
    ...
    public function piezaUtilizar($ot, $clase) //Función para obtener la pieza a utilizar.
    {
        ...
        for ($i = 0; $i < count($pzasEncontradas); $i++) { //Recorro las piezas encontradas.
            if (!array_search($pzasEncontradas[$i]->n_pieza, $pzasGuardadas)) {
                $numerosUsados = array();
                if (isset($pzasUsadas)) {
                    $numeroUsado = "";
                ...
                if (array_search($numero, $numerosUsados) === false) {
                    if (mb_substr($n_pieza, -1, null, 'UTF-8') == "M") { //Si la pieza es macho, se busca la pieza hembra.
                        $pieza = Pieza::where('id_ot', $ot)->where('id_clase', $clase->id)->where('proceso', 'Cepillado')->where('n_pieza', $numero . "H")->where(function ($query) {
                            $query->where(function ($q) {
                                $q->where('error', 'Ninguno')
                                    ->where('liberacion', 1);
                            })->orWhere(function ($q) {
                        ...
                        })->first(); //Busco la pieza hembra.
                    } else {
                        $pieza = Pieza::where('id_ot', $ot)->where('id_clase', $clase->id)->where('proceso', 'Cepillado')->where('n_pieza', $numero . "M")->where(function ($query) {
                            $query->where(function ($q) {
                                $q->where('error', 'Ninguno')
                                    ->where('liberacion', 1);
                            })->orWhere(function ($q) {
                        ...
                        })->first(); //Busco la pieza hembra. //Busco la pieza macho.
                    }
