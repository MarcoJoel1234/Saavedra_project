...
class PalomasController extends Controller
{
    public function show()
    {
        ...
        if (count($ot) != 0) {
            ...
            if (count($oTrabajo) != 0) {
                return view('processes.palomas', ['ot' => $oTrabajo]); //Retorno a vista de Palomas
            }
            //Se retorna a la vista de Palomas con las ordenes de trabajo que tienen clases que pasaran por Palomas
            return view('processes.palomas', ['ot']); //Retorno a vista de Palomas
        }
        ...
        if (isset($request->n_pieza)) {  //Si se obtienen los datos de las piezas, se guardan en la tabla Palomas_cnominal.
            ...
            if ($piezaExistente) {
                ...
                if (isset($pzaUtilizar)) { //Si existe una pieza para utilizar, se retorna a la vista de Offset
                    ...
                    return view('processes.palomas', ['band' => 2, 'moldura' => $moldura->nombre, 'ot' => $ot, 'meta' => $meta, 'cNominal' => $cNominal, 'tolerancia' => $tolerancia, 'nPiezas' => $pzasCreadas, 'clase' => $clase, 'piezaElegida' => $pzaUtilizar, 'juegos' => count($this->piezaUtilizar($ot->id, $clase))]); //Retorno a vista de Copiado
                } else {
        ...
        if ($id_proceso !== "[]") {
            ...
            if (isset($cNominal) && isset($tolerancia)) {
                ...
                if (isset($pzasUtilizar)) { //Si no se encontro una pieza para utilizar, se crea una nueva pieza.
                    return view('processes.palomas', ['band' => 2, 'moldura' => $moldura->nombre, 'ot' => $ot, 'meta' => $meta, 'cNominal' => $cNominal, 'tolerancia' => $tolerancia, 'nPiezas' => $pzasCreadas, 'clase' => $clase, 'piezasUtilizar' => $pzasUtilizar, 'juegos' => count($pzasUtilizar)]); //Retorno a vista de Palomas
                } else {
                ...
                $pzasUtilizar = $this->piezaUtilizar($ot->id, $clase); //Llamado a función para obtener las piezas disponibles.
                return view('processes.palomas', ['band' => 2, 'moldura' => $moldura->nombre, 'ot' => $ot, 'meta' => $meta, 'cNominal' => $cNominal, 'tolerancia' => $tolerancia, 'nPiezas' => $pzasCreadas, 'clase' => $clase, 'juegos' => count($pzasUtilizar)])->with('success', 'Se han registrado todas las piezas correctamente'); //Retorno a vista de Palomas
            }
        }
    }
            ...
            if (isset($request->password)) { //Si se ingreso una contraseña y la meta existe entonces...
                ...
                foreach ($usersPasswords as $userPassword) { //Recorro las contraseñas.
                    if (Hash::check($request->password, $userPassword->contrasena) && $userPassword->perfil == 1) {  //Si la contraseña es correcta.
                        return view('processes.palomas', ['band' => 4, 'moldura' => $moldura->nombre, 'ot' => $ot, 'meta' => $meta, 'clase' => $clase, 'cNominal' => $cNominal, 'tolerancia' => $tolerancia, 'nPiezas' => $pzasCreadas, 'juegos' => count($this->piezaUtilizar($ot->id, $clase))]); //Retorno la vista de Palomas
                    }
                }
            }
            $pzaUtilizar = Palomas_pza::where('id_proceso', $id_proceso->id)->where('estado', 1)->where('id_meta', $meta->id)->first(); //Obtención de la pieza a utilizar.
    ...
    public function piezaUtilizar($ot, $clase) //Función para obtener la pieza a utilizar.
    {
        ...
        $id_proceso = "palomas_" . $clase->nombre . "_" . $ot;
        $proceso = Palomas::where('id_proceso', $id_proceso)->first();
        $pzasOcupadas = Palomas_pza::where('id_proceso', $proceso->id)->where('estado', 1)->get(); //Obtención de todas las piezas creadas.
        if ($proceso) {
            $pzasUsadas = Pieza::where('id_ot', $ot)->where('id_clase', $clase->id)->where('proceso', 'Palomas')->get(); //Obtención de todas las piezas creadas en Palomas
        }

        if ($procesos->offSet != 0) {
            //Obtener las piezas solamente en el proceso de Palomas
            $pzasEncontradas = Pieza::where('id_ot', $ot)->where('id_clase', $clase->id)->where('proceso', 'Off Set')->where('error', 'Ninguno')->get();
    ...
    public function piezasEncontradas($pzasEncontradas, &$pzasUtilizar, &$pzasGuardadas, $pzasUsadas, $pzasOcupadas)
    {
        ...
        for ($i = 0; $i < count($pzasEncontradas); $i++) { //Recorro las piezas encontradas de Palomas
